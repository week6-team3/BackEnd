...
class UserController {
  ...
  createUserController = async (req, res) => {
    try {
      const { nickname, email, password, confirm } = await userSchema.validateAsync(req.body).catch((error) => {
        // joi error msg
  ...
  loginUserController = async (req, res) => {
    try {
      ...
      // 빈 값만 확인 ..
      const { email, password } = await loginSchema.validateAsync(req.body).catch((error) => {
        // joi error msg
      ...
      loginUserResult.errorMessage
        ? res.status(200).send({ errorMessage: loginUserResult.errorMessage })
        : res.status(200).send({ nickname: loginUserResult.usernickname });
    } catch (error) {
      console.log(error);
      res.status(400).send({ errorMessage: error.message });
    }
